...
# File logo này phải được đặt CÙNG THƯ MỤC với file code (app.py) trên VPS
...
# Kiểm tra xem file logo mặc định có nằm đúng chỗ không trước khi hiển thị web
if not os.path.exists(DEFAULT_LOGO_PATH):
    # Hiển thị thông báo lỗi màu đỏ nếu mất file logo
    st.error(
        f"⚠️ Lỗi hệ thống: Không tìm thấy file logo mặc định tại `{DEFAULT_LOGO_PATH}`. Vui lòng kiểm tra lại server!")
else:
    ...
    with col2:
        ...
        # Ô upload cho phép chọn nhiều ảnh cùng lúc, hỗ trợ png, jpg, jpeg, webp
        bg_files = st.file_uploader("Kéo thả các ảnh cần gắn Logo vào đây", type=['png', 'jpg', 'jpeg', 'webp'],
    ...
    if bg_files:
        try:
            # Tự động tải file logo từ server
            ...
            if st.button("🚀 Bắt đầu xử lý ảnh", type="primary"):
                ...
                with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
                    ...
                    for idx, bg_file in enumerate(bg_files):
                        ...
                        if idx == 0:
                            st.image(final_img, caption="Ảnh Preview (Ảnh đầu tiên)", use_column_width=True)
